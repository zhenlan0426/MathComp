######################################################################################################################################
######### TODO: for train_policy.py remove '[completed]' from finished path or not remove, it can be used for long path #########
######################################################################################################################################
...
def filter_input(batch_response,current_level_node):
    ...
    # mark finished path as completed
    inputs_minus1 = [update_steps(prm_input,None) if IsFinished(prm_input) else update_steps(prm_input, max_depth-current_level) for prm_input in prm_inputs]
    return prm_inputs,inputs_minus1,parents,len(prm_inputs)
...
def filter_inputs(batch_responses,current_level_nodes,lengths):
    ...
    return prm_inputs,input_minus1_list,parent_list,lengths
...
while (current_level < max_depth) and (current_level_nodes):
    ...
    current_level += 1
    prm_inputs,inputs_minus1,parent_list,lengths = filter_inputs(batch_responses,current_level_nodes,lengths)
