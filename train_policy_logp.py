...
data = []
count_1 = 0
for y,score,text,code,prob_i,exit_i,logp in completed_paths_y:
    data.append([text,y,logp])
    if y == 1: count_1 += 1
texts,ys,logps = zip(*data)
ys = np.array(ys)
...
for text,y,logp in zip(texts,ys,logps):
    ...
    y_final.append(y)
    logp_final.append(logp[len(question)-1:-1])
    assert len(question+answer)==(len(logp)+1)

# Pi
with open(f"../llmOutputs/PRM/data_pi1_code{version}.pickle", "rb") as f:
    data_pi = pickle.load(f)
texts2,ys2,lengths_raw,logps2 = zip(*data_pi)
ys2 = np.array(ys2)
ys2 = ys2/ys2.std()

for text,idx,y,logp in zip(texts2,lengths_raw,ys2,logps2):
    question = tokenizer.encode(text[:idx],add_special_tokens=True)
    answer = tokenizer.encode(text[idx:],add_special_tokens=False)
    if len(question) > 1100: continue # at least 1200 - 1100 to train on
    lengths.append(len(question))
...
if int(version) > 1: # correct_paths does not exist for version 1
    ...
    for _,v in data_pre.items(): # (prob_i -> [(sol1,logp1),...])...
        text,logp = random.choice(v) # one sample sol per question
        idx = search_patterns(text)
        question = tokenizer.encode(text[:idx],add_special_tokens=True)
        answer = tokenizer.encode(text[idx:],add_special_tokens=False)
        if len(question) > 1100: continue # at least 1200 - 1100 to train on
    ...
    ys = y_final
assert len(ys) == len(input_ids) == len(lengths) == len(logp_final)
...
# check calculations
temp = []
for text,l in from_gen(input_ids[:100],lengths[:100]):
    with torch.no_grad():
        logits = model(text).logits[0,l:-1]
        logP = logP_from_logits(logits, text[0,l+1:]).cpu().numpy()
    temp.append(logP)
print([np.mean(np.abs(i-j)) for i,j in zip(temp,logp_final[:100])])

# get old logP
start = len(logp_final) - len(input_ids)
for text,l in from_gen(input_ids[start:],lengths[start:]):
    with torch.no_grad():
        logits = model(text).logits[0,l:-1]
        logP = logP_from_logits(logits, text[0,l+1:]).cpu().numpy()
    logp_final.append(logP)
assert len(logp_final) == len(input_ids)

data = list(zip(input_ids,ys,lengths,logp_final))
random.shuffle(data)
input_ids,ys,lengths,logp_final = list(zip(*data))

# save completed_paths_y_code in correct_paths.json
data = pd.DataFrame(completed_paths_y,columns=['isCorrect','score','node','code','prob_i','exit_i','logps'])
data['prob_i'] = data['prob_i'].astype(str)
# prob_i -> [(sol1,logp1),...]
data_dic = data.groupby('prob_i').apply(lambda x: list(zip(x['node'], x['logps']))).to_dict()
...
for epoch in range(epochs):
    for i,(text,logP_old,adv,l) in enumerate(from_gen(input_ids,logp_final,ys,lengths)):
        if i > 0:
